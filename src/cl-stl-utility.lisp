...
(locally (declare (optimize speed))
  (defun make_pair (first second)
	(let ((arr (make-array 2 :initial-element nil)))
	  (declare (type (cl:simple-vector 2) arr))
	  (_= (svref arr 0)  first)
	...
	(with-reference (rm)
	  (let ((pr rm))
		...
		(let ((arr1 (__inner-array pr))
		  ...
		  (declare (type (cl:simple-vector 2) arr1 arr2))
...
(locally (declare (optimize speed))
  ...
  (defun first (pr)
	(svref (the (cl:simple-vector 2) (__inner-array pr)) 0))
  
  (defun (setf first) (new-val pr)
	(setf (svref (the (cl:simple-vector 2) (__inner-array pr)) 0) new-val))

  (defun second (pr)
	(svref (the (cl:simple-vector 2) (__inner-array pr)) 1))

  (defun (setf second) (new-val pr)
	(setf (svref (the (cl:simple-vector 2) (__inner-array pr)) 1) new-val)))
